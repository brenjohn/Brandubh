...
This file defines functions which can be used to draw menu screens and a game 
board for playing brandubh.

These functions draw and print to a curses standard screen (stdscr).
"""
...
TITLE_LENGTH = max([len(line) for line in TITLE_LINES])
SUBTITLE = "F\u00e1ilte go brandubh / Welcome to brandubh"
...
def init_view(stdscr):
    """
    Initialises the colours to get curses to use when rendering different
    screens. It also sets the cursor to be invisible.
    """
...
def draw_main_menu(stdscr, option):
    """
    Draws the main menu for the game and highlights the given option. The menu
    screen consists of a large brandubh title and a subtitle. It also has
    options the user can select from, allowing them to play a game, read the
    rules or quit.
    """
    ...
    subtitle_x = int((width // 2) - (len(SUBTITLE) // 2) - len(SUBTITLE) % 2)
    title_y = 2
    ...
    stdscr.addstr(title_y + i + 1, subtitle_x, SUBTITLE[:width-1])
    stdscr.attroff(curses.color_pair(2)) 
    stdscr.attroff(curses.A_BOLD)
    
    # Print the available options with the given option highlighted.
    x = int((width // 2) - 8)
    for op, option_string in enumerate(OPTIONS):
...
def draw_game_screen(stdscr, game_pieces, 
    ...
    brandubh. To draw the game screen, this function expects as arguments:
        ...
        game_pieces          - A dict mapping board squares to the pieces that
                               occupy them.
                      
        (cursor_x, cursor_y) - The location of the cursor used by the user to 
                               select squares.
                               
        player               - Which player is to make the next move.
        
        next_move            - An array containing squares making up the next 
                               move selected by the user.
        
        previous_moves       - An array of the most recent moves played, to be
                               displayed on the screen.
        
        info_message         - A message to be printed to the screen regarding
                               illegal moves etc.
    """
    # Clear the screen before drawing anything.
    stdscr.clear()
    ...
    draw_board(stdscr, centre_x, top_y)
    draw_left_panel(stdscr, centre_x, top_y, player, *next_move)
    draw_right_panel(stdscr, centre_x, top_y, previous_moves)
    draw_bottom_panel(stdscr, centre_x, top_y, info_message)
    draw_game_pieces(stdscr, centre_x, top_y, game_pieces)
    draw_cursor(stdscr, cursor_x, cursor_y, centre_x, top_y)
...
def draw_board(stdscr, centre_x, top_y):
    """
    ...
    for i, ch in enumerate("1234567"):
        x, y = square_position(0, 6, centre_x, top_y)
        stdscr.addch(y+2, x+1+4*i, ch)
        
        
        
def draw_left_panel(stdscr, centre_x, top_y, player, piece=None, target=None):
    """
    ...
    draw_panel_border(stdscr, centre_x-31, top_y, 11, 13)
    
    # Print the panel header indicating who is making the next move and body
    # displaying the next move which moves the 'piece' to the 'target' square.
    # Coordinates for the piece and target square are translated into board
    # coordinates.
...
def draw_right_panel(stdscr, centre_x, top_y, previous_moves):
    """
    Draws the panel on the right of the board which is used to display the
    most recent moves of the game.
    """
    # Draw the panel border.
    draw_panel_border(stdscr, centre_x+20, top_y, 11, 13)
...
def draw_bottom_panel(stdscr, centre_x, top_y, message=""):
    """
    Draws the panel below the board which is used to print messages regarding
    illegal moves and to indicate special keys the user can press to pass a
    turn or end the game.
    """
    # Draw the panel border and print the relevant info.
    draw_panel_border(stdscr, centre_x-31, top_y+16, 62, 3)
...
def draw_panel_border(stdscr, x, y, w, h):
    """
    Draws a rectangle whose upper left corner is located at (x, y) and whose
...
def draw_game_pieces(stdscr, centre_x, top_y, game_pieces):
    """
    Draws the game pieces in the locations indicated by the dict 'game_pieces'.
    """
    stdscr.attron(curses.A_BOLD)
    for position, piece in game_pieces.items():
        if piece != 0: # Skip empty squares. 
            square_x, square_y = position
            x, y = square_position(square_x, square_y, centre_x, top_y)
            piece_symbol = GAME_PIECES[piece]
...
def draw_cursor(stdscr, cursor_x, cursor_y, centre_x, top_y):
    """
    Highlights the board square indicated by (cursor_x, cursor_y)
    """
    x, y = square_position(cursor_x, cursor_y, centre_x, top_y)
    for i in range(3):
...
def draw_game_over_banner(stdscr, winner):
    """
    Draws a rectangle above the board stating the game is over and prints who
    won the game.
    """
    ...
    draw_panel_border(stdscr, centre_x-12, 1, 24, 1)
...
def square_position(square_x, square_y, centre_x, top_y):
    """
